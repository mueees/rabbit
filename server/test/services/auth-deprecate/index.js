...
describe('Auth service', function () {
    ...
    describe("remove expired tokens", function () {

        it("should exist removeExpiredTokens method", function () {
            assert.isDefined(api.removeExpiredTokens);
        });

        describe("should remove expired tokens", function () {
            /*
             * Prepare:
             * Create user
            ...
            var tokens = [{
                    token: "fake token",
                    token_to_update: "fake token to update",
                    date_expired: (new Date()).setDate(-20)
                }],
                userData;
            ...
            beforeEach(function (done) {
                helpers.db.user.clearUsers()
                    ...
                    .then(function (user) {
                        helpers.db.user.addTokens(user, tokens).then(function (user) {
                            userData = user;
            ...
            it("should doesn't have any tokens", function () {
                assert.equal(userData.tokens.length, 0);
            });
